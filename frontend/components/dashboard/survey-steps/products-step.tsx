                ...
                <div key={item.id} className="relative rounded-lg border bg-slate-50/50 p-6 dark:bg-slate-900/50">
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <FormField
                            ...
                            render={() => (
                                <FormItem className="md:col-span-2">
                                    <FormLabel>模型选型偏好 (多选)</FormLabel>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 mt-2">
                                        {["依赖商用闭源API", "开源模型本地化部署", "自研专属大模型", "混合模式"].map((opt) => (
                                            <FormField
                                                key={opt}
                                                control={control}
                                                name={`products.${index}.model_preference`}
                                                render={({ field }) => (
                                                    <FormItem className="flex flex-row items-center space-x-3 space-y-0 text-left rounded-md border p-3 bg-background hover:bg-slate-50 transition-colors">
                                                        <FormControl>
                                                            <Checkbox
                                                        ...
                                                        <FormLabel className="font-normal flex-1 cursor-pointer">{opt}</FormLabel>
                                                    </FormItem>
                                                )}
                                            />
                                        ))}
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem><FormLabel>平均交付周期 (月)</FormLabel><FormControl><Input type="number" placeholder="选填" {...field} /></FormControl><FormMessage /></FormItem>
                            )}
                        />
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                        <FormControl><SelectTrigger><SelectValue placeholder="请选择" /></SelectTrigger></FormControl>
                                        <SelectContent>
                                            <SelectItem value="subscription">SaaS订阅</SelectItem>
                                            <SelectItem value="project">项目制买断</SelectItem>
                                            <SelectItem value="pay_as_you_go">按量计费 (API等)</SelectItem>
                                        </SelectContent>
                                    </Select>
                                    <FormMessage />
                                </FormItem>
