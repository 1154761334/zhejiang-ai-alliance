...
**Q: 前端服务无法启动或无法访问？**
A: 如果 PM2 报错 `Could not find a production build`，请删除并重启开发服务器：

```bash
pm2 stop zjal-frontend
pm2 delete zjal-frontend
cd frontend
pm2 start npm --name "zjal-frontend" -- run dev -- -p 4000
```
