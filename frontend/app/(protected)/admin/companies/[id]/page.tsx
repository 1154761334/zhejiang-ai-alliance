...
export default async function CompanyCrmPage({ params }: CompanyPageProps) {
  ...
  try {
    // @ts-ignore
    companyData = await client.request(readItem('companies', params.id, {
      fields: [
        '*', 
        'products.*', 
        'case_studies.*', 
  ...
  try {
    // @ts-ignore
    investigations = await client.request(readItems('org_internal_investigations', {
  ...
  return (
    <>
      ...
      />
      <div className="mt-4">
        <CompanyCrmView companyId={params.id} initialCompanyData={companyData} />
      </div>
    </>
  );
}
